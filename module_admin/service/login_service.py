...
class LoginService:
    ...
    async def get_current_user(
        ...
        if token == redis_token:
            if AppConfig.app_same_time_login:
                await request.app.state.redis.set(
                    f"{RedisInitKeyConfig.ACCESS_TOKEN.get('key')}:{session_id}", redis_token,
    ...
    async def get_current_user_routers(cls, user_id: int, query_db: AsyncSession):
        ...
        user_router_menu = sorted(
            [row for row in query_user.get('user_menu_info') if row.menu_type in ['M', 'C']],
    ...
    async def get_sms_code_services(cls, request: Request, query_db: AsyncSession, user: ResetUserModel):
        ...
        if is_user:
            ...
            await request.app.state.redis.set(
                f"{RedisInitKeyConfig.SMS_CODE.get('key')}:{session_id}", sms_code,
