...
class InfoService:
    ...
    async def add_info_services(cls, query_db: AsyncSession, page_object: InfoModel):
        ...
        :return: 新增服务器校验结果
        """
        ssh = await InfoDao.get_info_detail_by_info(query_db, page_object)
        if ssh:
    ...
    async def edit_info_services(cls, query_db: AsyncSession, page_object: InfoModel):
        ...
        ssh_info = await cls.info_detail_services(query_db, edit_info.get('ssh_id'))
        if ssh_info:
        ...
        if page_object.ssh_ids:
            ...
            try:
                for ssh_id in ssh_id_list:
                    await InfoDao.delete_info_dao(query_db, InfoModel(sshId=ssh_id))
                await query_db.commit()
    ...
    @classmethod
    async def info_detail_services(cls, query_db: AsyncSession, ssh_id: int):
        """
        获取服务器详细信息service
        ...
        :return: 服务器D对应的信息
        """
        info = await InfoDao.get_info_detail_by_id(query_db, ssh_id=ssh_id)
        if info:
