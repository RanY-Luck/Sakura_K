...
class UserDao:
    ...
    async def get_user_list(
            cls, db: AsyncSession, query_object: UserPageQueryModel, data_scope_sql: str,
            is_page: bool = False
    ):
        """
        根据查询参数获取用户列表信息
        ...
        query = select(SysUser, SysDept) \
            ...
            SysUser.del_flag == 0,
            or_(
                SysUser.dept_id == query_object.dept_id, SysUser.dept_id.in_(
    ...
    async def get_user_role_allocated_list_by_role_id(
            cls, db: AsyncSession, query_object: UserRolePageQueryModel,
            is_page: bool = False
    ):
        """
        根据角色id获取已分配的用户列表信息
    ...
    async def get_user_role_unallocated_list_by_role_id(
        ...
        unallocated_user_list = await PageUtil.paginate(
            db, query, query_object.page_num, query_object.page_size,
            is_page
        )
