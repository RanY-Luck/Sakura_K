...
Revision ID: 0c6b6f89f810
Revises: 
Create Date: 2023-05-05 10:13:22.151381
...
def upgrade():
    ...
    sa.Column('is_delete', sa.Boolean(), nullable=True, comment='是否软删除'),
    ...
    sa.Column('disabled', sa.Boolean(), nullable=True, comment='是否禁用'),
    sa.Column('hidden', sa.Boolean(), nullable=True, comment='是否隐藏'),
    ...
    sa.Column('noCache', sa.Boolean(), nullable=True, comment='如果设置为true，则不会被 <keep-alive> 缓存(默认 false)'),
    sa.Column('breadcrumb', sa.Boolean(), nullable=True, comment='如果设置为false，则不会在breadcrumb面包屑中显示(默认 true)'),
    sa.Column('affix', sa.Boolean(), nullable=True, comment='如果设置为true，则会一直固定在tag项中(默认 false)'),
    sa.Column('noTagsView', sa.Boolean(), nullable=True, comment='如果设置为true，则不会出现在tag中(默认 false)'),
    sa.Column('canTo', sa.Boolean(), nullable=True, comment='设置为true即使hidden为true，也依然可以进行路由跳转(默认 false)'),
    sa.Column('alwaysShow', sa.Boolean(), nullable=True, comment='当你一个路由下面的 children 声明的路由大于1个时，自动会变成嵌套的模式，\n        只有一个时，会将那个子路由当做根路由显示在侧边栏，若你想不管路由下面的 children 声明的个数都显示你的根路由，\n        你可以设置 alwaysShow: true，这样它就会忽略之前定义的规则，一直显示根路由(默认 true)'),
    ...
    sa.Column('user_total_number', sa.Integer(), nullable=True, comment='用户总数'),
    sa.PrimaryKeyConstraint('id'),
    comment='角色表'
    )
    ...
    sa.Column('is_reset_password', sa.Boolean(), nullable=True, comment='是否已经重置密码，没有重置的，登录系统后必须重置密码'),
    sa.Column('last_ip', sa.String(length=50), nullable=True, comment='最后一次登录IP'),
    sa.Column('last_login', sa.DateTime(), nullable=True, comment='最后一次登录时间'),
    sa.Column('is_staff', sa.Boolean(), nullable=True, comment='是否为工作人员'),
    sa.Column('wx_server_openid', sa.String(length=255), nullable=True, comment='服务端微微信平台OpenId'),
    sa.PrimaryKeyConstraint('id'),
    comment='用户表'
    )
    ...
    sa.Column('platform', sa.String(length=8), nullable=True, comment='登陆平台'),
    sa.Column('login_method', sa.String(length=8), nullable=True, comment='认证方式'),
    ...
    sa.Column('disabled', sa.Boolean(), nullable=True, comment='字典状态，是否禁用'),
    ...
    sa.Column('tab_label', sa.String(length=255), nullable=True, comment='tab标题'),
    ...
    op.create_index(op.f('ix_vadmin_system_settings_tab_id'), 'vadmin_system_settings_tab', ['id'], unique=True)
    ...
    op.create_index(op.f('ix_vadmin_help_issue_category_id'), 'vadmin_help_issue_category', ['id'], unique=True)
    ...
    sa.Column('order', sa.Integer(), nullable=True, comment='字典排序'),
    ...
    sa.Column('delete_datetime', sa.DateTime(), nullable=True, comment='删除时间'),
    sa.Column('is_delete', sa.Boolean(), nullable=True, comment='是否软删除'),
    sa.Column('config_label', sa.String(length=255), nullable=True, comment='配置表标签'),
    ...
    sa.Column('disabled', sa.Boolean(), nullable=True, comment='是否禁用'),
    sa.Column('tab_id', sa.Integer(), nullable=True, comment='关联tab标签'),
    ...
    op.create_index(op.f('ix_vadmin_system_settings_id'), 'vadmin_system_settings', ['id'], unique=True)
    ...
    sa.Column('delete_datetime', sa.DateTime(), nullable=True, comment='删除时间'),
    sa.Column('is_delete', sa.Boolean(), nullable=True, comment='是否软删除'),
    sa.Column('category_id', sa.Integer(), nullable=True, comment='类别'),
    ...
    op.create_index(op.f('ix_vadmin_help_issue_id'), 'vadmin_help_issue', ['id'], unique=True)
...
def downgrade():
    ...
    op.drop_index(op.f('ix_vadmin_help_issue_id'), table_name='vadmin_help_issue')
    op.drop_table('vadmin_help_issue')
    op.drop_index(op.f('ix_vadmin_system_settings_id'), table_name='vadmin_system_settings')
    ...
    op.drop_index(op.f('ix_vadmin_help_issue_category_id'), table_name='vadmin_help_issue_category')
    ...
    op.drop_index(op.f('ix_vadmin_system_settings_tab_id'), table_name='vadmin_system_settings_tab')
