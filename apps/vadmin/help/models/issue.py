...
class VadminIssue(BaseModel):
    """
    代码解释：
    定义了一个名为VadminIssue的Pydantic模型类，用于表示常见问题记录。
    在该类中，__tablename__属性指定了此模型类对应的数据库表名，__table_args__属性指定了表名的注释信息。
    该类中包含了七个属性，分别为category_id、categort、title、content、view_number、is_active和user_id。
    这些属性通过Column函数映射到数据库表结构中，包括了数据类型、索引、非空约束、注释等信息。
    其中，category_id属性指定了该问题所属的类别，属于整数类型，在vadmin_help_issue_category表中某一行的ID；
    categort属性是一个关系属性，表示Issue模型与Category模型之间的关联关系，通过ForeignKey和relationship函数建立，实现一个Category对应多个Issue；
    title属性指定了问题的标题，属于255个字符长度的字符串类型，需要建立索引且不允许为空；
    content属性指定了问题的详细内容，属于Text类型，允许为空；
    view_number属性指定了问题的查看次数，属于整数类型，在表中默认值为0；
    is_active属性指定了问题是否可见，属于布尔类型，在表中默认值为True；
    user_id属性指定了创建该问题的用户ID，属于整数类型，在vadmin_auth_user表中某一行的ID。
    此外，该模型类中还定义了一个名为user的关系属性，表示Issue模型与User模型之间的关联关系。
    关联关系通过ForeignKey和relationship函数建立，在数据库中通过外键进行关联查询。
    """
