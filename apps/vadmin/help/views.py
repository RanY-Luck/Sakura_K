...
@app.get("/issue/categorys/platform/{platform}/", summary="获取平台中常见问题类别列表")
async def get_issue_category_platform(
    ...
    #
    model = models.VadminIssueCategory
    options = [joinedload(model.issue)]
...
async def get_issues(
    ...
    options = [joinedload(model.user), joinedload(model.categort)]
...
@app.post("/issues/", summary="批量删除问题", description="硬删除")
async def delete_issue(
        ids: IdList = Depends(),  # 自定义的参数类型IdList，表示要删除的问题ID列表
        auth: Auth = Depends(AllUserAuth())  # 身份验证
):
    # 调用crud.IssueDal实例的delete_datas()方法删除问题数据，传递的参数是ids=ids.ids（表示要删除的问题ID列表）和v_soft=False（表示硬删除）。
    # delete_datas()方法没有返回值，只是对数据库进行修改操作。
    await crud.IssueDal(auth.db).delete_datas(ids=ids.ids, v_soft=False)
    # 最后，路由函数返回一个SuccessResponse对象，其中包含一个字符串，表示删除成功。
    # SuccessResponse是一个自定义的响应类，它包含一个属性data，表示返回的数据。
    return SuccessResponse("删除成功")
...
async def put_issue(
        data_id: int,  # 表示要更新的问题的ID
        data: schemas.Issue,  # 自定义的数据类型schemas.Issue，表示要更新的问题的新信息
        auth: Auth = Depends(AllUserAuth())  # 身份验证
):
    # 调用crud.IssueDal实例的put_data()方法更新问题数据，传递的参数是data_id（表示要更新的问题的ID）和data（表示要更新的问题的新信息）。
    # put_data()方法返回一个更新后的问题数据。
    # 最后，路由函数返回一个SuccessResponse对象，其中包含更新后的问题数据。
    # SuccessResponse是一个自定义的响应类，它包含一个属性data，表示返回的数据。
...
async def get_issue(
        data_id: int,  # 获取的问题的ID
        db: AsyncSession = Depends(db_getter)  # 一个异步的数据库会话对象，由一个名为"db_getter"的依赖注入函数提供。
):
    #  定义了一个变量"schema"，它指定了返回结果的数据模型，即"schemas.IssueSimpleOut"，表示获取问题信息的简化版数据模型。
    schema = schemas.IssueSimpleOut
    # 函数处理器调用了一个名为"get_data"的异步函数，该函数由"crud.IssueDal"模块提供，用于从数据库中获取指定ID的问题信息，
    # 并返回一个SuccessResponse对象，其中包含获取到的问题信息的简化版数据模型。
...
async def issue_add_view_number(
        data_id: int,  # 表示要更新的问题的ID
        db: AsyncSession = Depends(db_getter)  # 一个异步的数据库会话对象，由一个名为"db_getter"的依赖注入函数提供。
):
    # 函数处理器是一个异步函数"issue_add_view_number"，它接受一个"data_id"参数，表示要更新的问题的ID，还接受一个依赖注入的参数"db"，
    # 它是一个异步的数据库会话对象，由一个名为"db_getter"的依赖注入函数提供。
    # 在函数处理器中，函数调用了一个名为"add_view_number"的异步函数，该函数由"crud.IssueDal"模块提供，
    # 用于更新指定ID的问题的查看次数加1，并返回更新后的问题信息。
    # 函数处理器将函数返回值封装成一个SuccessResponse对象，表示更新操作成功。
