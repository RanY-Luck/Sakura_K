...
async def login_for_access_token(
        request: Request,
        data: LoginForm,  # 自定义的数据模型LoginForm，用于接收用户提交的登录信息
        manage: LoginManage = Depends(),  # 处理登录逻辑
        db: AsyncSession = Depends(db_getter)  # 数据库访问
):
    """
    ...
    try:
        ...
        await VadminLoginRecord.create_login_record(db, data, False, request, {"msssage": str(e)})
...
@app.get("/getMenuList/", summary="获取当前用户的菜单列表")
...
@app.post("/token/refresh/", summary="刷新访问令牌[Token]")
