...
class UserDal(DalBase):
    ...
    async def update_current_info(self, user: models.VadminUser, data: schemas.UserUpdateBaseInfo):
        ...
        if data.telephone != user.telephone:
            unique = await self.get_data(telephone=data.telephone, v_return_none=True)
            if unique:
                raise CustomException("手机号码已存在！", code=status.HTTP_ERROR)
            else:
        ...
        for user in datas:
            ...
            for item in header:
                ...
                if field == "is_active":
                    ...
                    result = list(filter(lambda i: i["vallue"] == value, options["sys_vadmin_gender"]))
    ...
    async def get_import_headers_options(self):
        ...
        # 性别选项
        dict_types = await vadminSystemCRUD.DictTypeDal(self.db).get_dicts_details(["sts_vadmin_gender"])
        gender_options = self.import_headers[3]
        assert isinstance(gender_options, dict)
        sts_vadmin_gender = dict_types.get("sys_vadmin_gender")
        gender_options["options"] = [{"label": item["label"], "value": item["value"]} for item in sts_vadmin_gender]
    ...
    async def update_wx_server_openid(self, code: str, user: models.VadminUser, rd: Redis):
        """
        更新用户服务端微信平台openid
        :param code:
        :param user:
...
class RoleDal(DalBase):
    ...
    async def get_role_menu_tree(self, role_id: int):
        ...
        :return:
        代码解释：
        调用 get_data 方法获取指定 ID 的角色实例对象，通过传入 v_options 参数，
        使用 joinedload 方法将数据关联的菜单信息预加载，提高查询性能。
        然后，直接返回 role.menus 中所有菜单对象的 id 属性构成的列表。
        """
...
class MenuDal(DalBase):
    ...
    async def get_routers(self, user: models.VadminUser):
        ...
        if any([i.is_admin() for i in user.roles]):
            ...
            user = await UserDal(self.db).get_datas(user.id, v_options=options)
            datas = set()
    ...
    async def delete_datas(
        ...
        for obj in objs:
            if obj.roles:
                raise CustomException("无法删除存在的角色关联的菜单", code=400)
