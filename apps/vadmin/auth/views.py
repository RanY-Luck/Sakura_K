...
@app.delete("/users/", summary="批量删除用户", description="软删除，删除后会清空所关联角色")
...
@app.put("/user/{data_id}/", summary="更新用户信息")
async def put_user(
        data_id: int,  # 从URL路径中获取的待更新用户的ID
        data: schemas.UserUpdate,  # 请求体中的数据，其类型为UserUpdate，包含了需要更新的用户信息
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.user.update"]))  # 用于身份验证
):
...
async def user_current_rest_password(
...
@app.get("/user/download/import/template", summary="下载最新批量导入用户模版")
...
async def get_role_options(
        ...
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.role.create", "auth.user.update"]))
):
...
async def get_menus(
        ...
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.create", "auth,menu.update"]))
):
...
async def create_menu(
        menu: schemas.Menu,  # 请求体传入的菜单信息
        # 身份验证，要求验证权限为"auth.menu.create"的FullAdminAuth。
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.create"]))
):
    # 如果菜单信息中指定了父级菜单(parent_id不为None)，则将菜单信息中的alwaysShow属性设置为False，以保证在前端展示时，该菜单的子菜单会默认折叠（不显示）。
    if menu.parent_id:
        menu.alwaysShow = False
    # 返回一个成功响应对象（SuccessResponse），其中data字段为await crud.MenuDal(auth.db).create_data(data=menu)的返回结果，即创建成功的菜单信息。
    # 在创建前，该代码会根据Auth中的验证信息，判断该用户是否具有"auth.menu.create"的权限。
    # 如果验证通过，则调用crud.MenuDal类中的create_data方法，用于将传入的菜单信息保存到数据库，并返回创建成功的菜单信息。
    # 最终将成功响应对象返回给客户端。
...
async def delete_menus(
        ids: IdList = Depends(),  # 获取需要删除的菜单id列表
        # 身份验证，要求验证权限为"auth.menu.delete"的FullAdminAuth。
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.delete"]))
):
    # 需要删除传入的所有菜单id所对应的菜单。在删除前，该代码会根据Auth中的验证信息，判断该用户是否具有"auth.menu.delete"的权限。
    # 如果验证通过，则调用crud.MenuDal类中的delete_datas方法，将传入的菜单id列表作为参数，用于从数据库中硬删除这些菜单的信息。
    # 若该菜单存在角色关联，则无法删除。v_soft=False表示进行硬删除，即完全从数据库中彻底删除菜单信息。
    # 最终将成功响应对象返回给客户端。
...
async def put_menus(
        ...
        # 通过请求体传入的菜单信息
        data: schemas.Menu,
        # 身份验证，要求验证权限为"auth.menu.update"的FullAdminAuth。
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.update"]))
):
    # 返回一个成功响应对象（SuccessResponse），其中data字段为await crud.MenuDal(auth.db).put_data(data_id, data)的返回结果，即更新成功的菜单信息。
    # 在更新前，该代码会根据Auth中的验证信息，判断该用户是否具有"auth.menu.update"的权限。
    # 如果验证通过，则调用crud.MenuDal类中的put_data方法，用于将传入的菜单信息保存到数据库，并返回更新成功的菜单信息。
    # 最终将成功响应对象返回给客户端。
...
async def put_menus(
        data_id: int,  # 菜单信息的ID
        # 身份验证，要求验证权限为"auth.menu.view"或"auth.menu.update"的FullAdminAuth。
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.menu.view", "auth.menu.update"]))
):
    # 在获取前，该代码会根据Auth中的验证信息，判断该用户是否具有"auth.menu.view"或"auth.menu.update"的权限。
    # 如果验证通过，则调用crud.MenuDal类中的get_data方法，用于从数据库中获取指定ID的菜单信息，并且使用schemas.MenuSimpleOut来解析获取到的数据。
    # 最终将成功响应对象返回给客户端。
    schema = schemas.MenuSimpleOut
    # 返回一个成功响应对象（SuccessResponse），其中data字段为await crud.MenuDal(auth.db).get_data(data_id, None, v_schema=schema)的返回结果，即数据库中查找的菜单信息
...
async def get_role_menu_tree(
        role_id: int,  # 角色的ID
        # 身份验证，要求验证权限为"auth.role.create"或"auth.role.update"的FullAdminAuth。
        auth: Auth = Depends(FullAdminAuth(permissions=["auth.role.create", "auth.role.update"]))
):
    # 在获取前，该代码会根据Auth中的验证信息，判断该用户是否具有"auth.role.create"或"auth.role.update"的权限。
    # 如果验证通过，则调用crud.MenuDal类中的get_tree_list方法，用于从数据库中获取所有菜单列表，并将菜单列表以树状结构返回。
    treeselect = await crud.MenuDal(auth.db).get_tree_list(mode=3)
    # 然后调用crud.RoleDal类中的get_role_menu_tree方法，用于获取指定角色的菜单权限列表信息。
    role_menu_tree = await crud.RoleDal(auth.db).get_role_menu_tree(role_id)
    # 返回一个成功响应对象（SuccessResponse），其中data字段为{"role_menu_tree": role_menu_tree, "menus": treeselect}的字典类型
    # 最终将一个包含两个字段的字典类型返回给客户端，其中"role_menu_tree"表示获取到的角色菜单权限ID信息，"menus"表示从数据库中获取到的菜单列表树结构信息。
