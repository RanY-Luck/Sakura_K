...
class WriteXlsx:
    ...
    def __init__(self, filename: str = None, sheet_name: str = "sheet1"):
        ...
        file_dir = os.path.join(TEMP_DIR, data)
        if not os.path.exists(file_dir):
            os.mkdir(file_dir)
        if not filename:
            filename = hashlib.md5(str(random.random()).encode()).hexdigest()
        self.file_url = f"{TEMP_URL}/{data}/{filename}.xlsx"
        self.filename = os.path.join(TEMP_DIR, data, filename + ".xlsx")
