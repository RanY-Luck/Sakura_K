...
class MongoManage:
    ...
    async def get_db(self, _id: str = None, v_return_none: bool = False, v_schema: Any = None, **kwargs) -> dict | None:
        """
    ...
    async def create_data(self, data: dict | Any) -> InsertOneResult:
        ...
        if result.acknowledged:
            ...
            raise CustomException("创建数据失败", code=status.HTTP_ERROR)
    ...
    async def delete_data(self, _id: str):
        ...
        if result.deleted_count > 0:
            ...
            raise CustomException("删除失败，未查找出对应数据", code=status.HTTP_404_NOT_FOUND)
